...
import { z } from "zod";
import { redisUpstashClient } from "../redis.old";
...
export function getUserFromSession(cookies: Pick<Cookies, "get">) {
  ...
  const userSessionId = getUserSessionById(sessionId);
...
export async function updateUserSessionData(
  ...
  await redisUpstashClient.set(`session:${sessionId}`, sessionSchema.parse(user), {
...
export async function updateUserSessionExpiration(
  ...
  await redisUpstashClient.set(`${SERVER_SESSION_KEY_PREFIX}${sessionId}`, user, {
    ex: SESSION_EXPIRATION_SECONDS,
  });
  setCookie(sessionId, cookies);
}
...
export async function removeUserFromSession(
  ...
  await redisUpstashClient.del(`${SERVER_SESSION_KEY_PREFIX}${sessionId}`);
...
async function getUserSessionById(sessionId: string) {
  const rawUser = await redisUpstashClient.get(
    `${SERVER_SESSION_KEY_PREFIX}${sessionId}`
  );
